...
```{r load-in}
# Load in ICR data
data = read.csv("Data/Processed_RC2_SPS_11-2021_Data.csv", check.names = F, row.names = 1)
mol = read.csv("Data/Processed_RC2_SPS_11-2021_Mol.csv", check.names = F, row.names = 1)

# Load in metadata
meta1 = read.csv("v3_SFA_SpatialStudy_2021_SampleData/SPS_Sample_Field_Metadata.csv")
meta2 = read.csv("v3_SFA_SpatialStudy_2021_SampleData/SPS_Sample_IGSN-Mapping.csv", skip = 1) %>% dplyr::select(Site_ID = 'Locality', Location = 'Physiographic_Feature_Name' )%>% distinct()
...
geo.files = list.files(path = "Geospatial_Data/", pattern = "updated", full.names = T)
...
geochem = read.csv("v3_SFA_SpatialStudy_2021_SampleData/v3_SPS_Water_Sample_Data_Summary.csv", skip = 2)
...
# Load in MCD
mcd = read.tree("Data/RC2_SPS_03-2022_MCD_UPGMA.tre")
...
bnti = as.matrix(as.dist(read.csv("Data/RC2_SPS_MCD_bNTI_999.csv", row.names = 1)))
